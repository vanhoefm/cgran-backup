...
class qa_pr_stream_to_pvec(gr_unittest.TestCase):
	...
	def test_001_stream_to_pvec(self):
		...
		data = range(size) + range(2,2+size)
		expected = range(size) + [0]*(block-size) + range(2,2+size) + [0]*(block-size)
		
		bsrc = gr.vector_source_b(data)
		uut = pr.stream_to_pvec(gr.sizeof_char, size)
		bv2s = gr.vector_to_stream(gr.sizeof_char, block)
