    ...
    d_pending_flag(0)
{
  ...
  for (size_t i = 0; i < d_preamble_nc.size(); i++){
    if (d_preamble_nc[i].size() != (size_t) d_fft_length)
...
void
gr_ofdm_insert_preamble::reset_preamble(std::vector<std::vector<gr_complex> > new_preamble)
{
