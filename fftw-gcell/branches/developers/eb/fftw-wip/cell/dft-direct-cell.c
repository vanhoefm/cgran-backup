...
typedef struct {
     ...
     int Wsz;
     R *W;
     int nspe;	  /* # of spes to use on this subproblem */
...
static void print(const plan *ego_, printer *p)
{
     const P *ego = (const P *) ego_;
     int i;
     p->print(p, "(dft-direct-cell-%D/%ds%d", ego->n, ego->cutdim, ego->nspe);
     for (i = 0; i < 2; ++i)
...
static solver *mksolver(int cutdim, int nspe)
{
...
static char nspe_table[] = { 1, 2, 3, 4, 6, 8, 12, 16, 0};

void X(dft_direct_cell_register)(planner *p)
{
     int i, n;

     for (i = 0; (n = nspe_table[i]) != 0; i++){
	  REGISTER_SOLVER(p, mksolver(0, n));
	  REGISTER_SOLVER(p, mksolver(1, n));
     }
}
		    ...
		    planner *plnr)
{
     ...
     if (NO_SIMDP(plnr) || ego->nspe > X(cell_nspe)())
	  return 0;
...
static void regsolverw(planner *plnr, INT r, int dec, int cutdim, int nspe)
{
...
void X(ct_cell_direct_register)(planner *p)
{
     ...
     for (n = 0; n <= MAX_N; n += REQUIRE_N_MULTIPLE_OF) {
	  ...
	  if (r->r[0]) {
	       int j, nspe;
	       for (j = 0; (nspe = nspe_table[j]) != 0; j++){
		    regsolverw(p, n, DECDIT, 0, nspe);
		    regsolverw(p, n, DECDIT, 1, nspe);
		    regsolverw(p, n, DECDIF+TRANSPOSE, 0, nspe);
		    regsolverw(p, n, DECDIF+TRANSPOSE, 1, nspe);
	       }
	  }
     }
}
