...
#if defined(HAVE_CELL) && !defined(HAVE_GCELL)
...
{
  X(cell_spe_awake_some)(nspes);
  X(cell_spe_wait_some)(nspes);
  UNUSED(ctx);
}
...
{
  X(cell_spe_awake_some(s_nspe));
}

void X(cell_spe_wait_all)(void)
{
  X(cell_spe_wait_some(s_nspe));
}

void X(cell_spe_awake_some)(int nspe)
{
     int i;
#ifdef HAVE_LIBSPE2
     unsigned int zero = 0;
...
void X(cell_spe_wait_some)(int nspe)
{
     int i;
     for (i = 0; i < nspe; ++i) {
	  while (!ctx[i]->done)
