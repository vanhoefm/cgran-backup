...
% Date: 23/04/07
%**************************************************************************
function USRPF_set_rf_tx_freq(connection, rf_freq)
 
    command_id= 'set_rf_tx_freq';
 
    if rf_freq < 400e6 || rf_freq > 500e6
        error('RF transmit frequency must be 400 to 500 Hz')
    end
 
    command = [command_id, ':', num2str(rf_freq)];
    success = MEX_USRPF_send_string(connection, command);
    if success == 0
        error('Unable to set RF transmit frequency')
    end
