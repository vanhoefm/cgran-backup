/** simapp.cpp: Main wxWidgets application with GUI basics for NRLMQP intruder simulator
 *  
 * @author Brian Shaw
 * 
 */
...
bool NRLApp::OnInit(){
  ...
  NRLFrame* frame = new NRLFrame(_("NRLMQP Intruder Simulator"), this);
  
  panel = new wxPanel(frame, -1, wxPoint(0,100),
		      wxSize(GUI_IMAGE_WIDTH, GUI_IMAGE_HEIGHT + 100));
  ...
  wxStaticBitmap* backgroundbmp = new wxStaticBitmap(panel, wxID_STATIC, backgroundbitmap,
						     wxPoint(0, 0), wxSize(GUI_IMAGE_WIDTH, GUI_IMAGE_HEIGHT));
  //Show the frame
  frame->Show(true);
  SetTopWindow(frame);
  //Start event loop
