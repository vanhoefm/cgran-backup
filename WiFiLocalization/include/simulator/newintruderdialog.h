/** newintruderdialog.h: Dialog window for creating a new intruder
 * 
...
#ifndef NRL_NEW_INTRUDER_DIALOG_H
#define NRL_NEW_INTRUDER_DIALOG_H
...
class NewIntruderDialog: public wxDialog
{    
  DECLARE_EVENT_TABLE()
private:
  ...
  NRLApp* app;
  NewIntruderDialog** newintruderdialog; //Pointer to main frame's pointer to us. Used for determining if the dialog box is open.
  //For SquarePattern1
  Location patternvelocity; //The pattern code can handle any X, Y velocity.
  int patternvel; //Serves the same purpose as patternvelocity but is simpler.
		  //Always assumes the intruder begins by going NE
  int numsteps;
  int numsecs;
  
public:
  /// Default constructor and destructor. Use Create() to initialize everything.
  bool Create(wxWindow* parent, NRLApp* myapp, NewIntruderDialog** dialog, Intruder** newintruderptr);
};
