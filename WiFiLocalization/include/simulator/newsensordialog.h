/** newsensordialog.h: Dialog window for creating a new intruder
 * 
 * This implementation is based on the Personal Record example
 * @author Brian Shaw
 * @author Julian Smart
 * 
 */ //Note: Very similar to newintruderdialog!
/* 
...
#ifndef NRL_NEW_SENSOR_DIALOG_H
#define NRL_NEW_SENSOR_DIALOG_H
...
class NewSensorDialog: public wxDialog
{    
  DECLARE_EVENT_TABLE()
private:
  ...
  int32_t x;
  int32_t y;
  Location** sensorptr; //Pointer to memory space owned by the main frame
  NRLApp* app;
  NewSensorDialog** newsensordialog; //Pointer to main frame's pointer to us. Used for determining if the dialog box is open.
  int id;
public:
  /// Default constructor and destructor. Use Create() to initialize everything.
  bool Create(wxWindow* parent, NRLApp* myapp, NewSensorDialog** dialog, Location** newsensorptr);
};
